...
def build_vocab(tokenizer, file_path: str, vocab_path: str, vocab_size=20000):
    ...
    json.dump(
        word2id, open(vocab_path, "w", encoding="utf-8"), ensure_ascii=False, indent=4
    )
...
def preprocess(
    ...
    for line in lines:
        ...
        source_ids = [
            source_vocab.get(token, source_vocab["<UNK>"]) for token in source_tokens
        ]
        target_ids = [
            target_vocab.get(token, target_vocab["<UNK>"]) for token in target_tokens
        ]
...
if __name__ == "__main__":
    ...
    train_data, valid_data = preprocess(
        tokenizer, "data/raw/fra.txt", source_vocab_path, target_vocab_path
    )
