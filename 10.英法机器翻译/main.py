...
def parse_args():
    ...
    parser.add_argument(
        "--train_data_path", type=str, default="data/processed/train.json"
    )
    parser.add_argument(
...
if args.mode == "train":
    ...
    dataloader = DataLoader(
        MyDataset(args.train_data_path),
    ...
    # 加载优化器参数
    if os.path.exists(
        os.path.join(args.model_path, "optimizer_" + str(args.ckpt) + ".bin")
    ):
    ...
    # 加载调度器参数
    if os.path.exists(
        os.path.join(args.model_path, "lr_scheduler_" + str(args.ckpt) + ".bin")
    ):
    ...
    for epoch in range(args.num_epochs):
        ...
        torch.save(
            model.state_dict(),
            os.path.join(args.model_path, "model_" + str(epoch) + ".bin"),
        )
        # 保存优化器参数
        torch.save(
            optimizer.state_dict(),
            os.path.join(args.model_path, "optimizer_" + str(epoch) + ".bin"),
        )
        # 保存调度器参数
        torch.save(
            lr_scheduler.state_dict(),
            os.path.join(args.model_path, "lr_scheduler_" + str(epoch) + ".bin"),
        )
# 评估模式
else:
    model.eval()
