...
def preprocess():
    ...
    for item in raw_relation_data:
        ...
        relation_triplets.append((head2id[head], relation2id[relation], tail2id[tail]))

    print(
        f"{len(head2id)} head entities, {len(tail2id)} tail entities, {len(relation2id)} relations."
    )
