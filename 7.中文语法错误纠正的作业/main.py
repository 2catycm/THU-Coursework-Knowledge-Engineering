...
import torch
import re
from infra import GECDataset, load_vocab_dict
import einops
import numpy as np
...
for epoch in range(total_epoch):
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        train_loss = MA(
            train_loss,
            loss.item() if not np.isnan(loss.item()) else train_loss,
            batch_idx,
        )
        valid_labels = labels != -100
        correct += (
            (output.max(dim=-1).indices[valid_labels] == labels[valid_labels])
            .sum()
            .item()
        )
        ...
        print(
            f"\rEpoch {epoch} Step {batch_idx + 1}/{len(train_dataloader)}: {int(elasped_time / 60)}min {round(elasped_time) % 60}s | Loss = {train_loss:.4f} | Accuracy = {correct / total * 100:.4f}%    ",
            end="",
        )
    print()
